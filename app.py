...
DOCUMENT_TITLES = {
    "ai_strategic_decision_making.pdf": "Artificial Intelligence and Strategic Decision-Making",
    "corporate_management_in_age_of_ai.pdf": "Corporate Management in the Age of AI",
    ...
    st.session_state.messages.append(
        {
            "role": "assistant",
            "content": answer,
            "citations": citation_labels(verification.selected_evidence),
            "refused": False,
        }
    )


st.set_page_config(page_title="Student AI Library", page_icon="📚", layout="centered")
...
st.markdown(
    ...
    <style>
        .stApp {
            background: linear-gradient(145deg, #f7f9fc 0%, #eef3f8 52%, #f9fafc 100%);
            color: #1f2937;
        }
        .block-container {
            max-width: 900px;
        ...
        .hero p { color: #526170; margin-bottom: 0; }
        .grounded {
            background: #eaf4f1;
...
.stButton > button {
    ...
    box-shadow: 0 3px 10px rgba(31, 55, 80, 0.06) !important;
...
with st.sidebar:
    ...
    st.caption("Three locally indexed papers for grounded study and exploration.")
    for document in sorted(DOCUMENTS_DIRECTORY.glob("*.pdf")):
        title = DOCUMENT_TITLES.get(document.name, document.stem.replace("_", " ").title())
        st.markdown(f"**{title}**")
    ...
    st.markdown(
        "Local document retrieval finds relevant passages. Similarity-based verification "
        "checks the evidence before evidence-only generation. If the evidence is insufficient, "
...
st.markdown(
    """
    <section class="hero">
        <div class="eyebrow">Research companion · grounded RAG</div>
        <h1>Student AI Library</h1>
        <h3>Your grounded research assistant for business and management studies.</h3>
        <p>Ask questions about the research papers in this library. Answers are generated only from the provided documents.</p>
    </section>
    <div class="grounded"><strong>🔒 Grounded answers only</strong><br>
    Questions outside the document collection are refused so your study notes stay anchored to the source material.</div>
    """,
    unsafe_allow_html=True,
)

st.markdown('<div class="suggestion-label">Start with a suggested question</div>', unsafe_allow_html=True)
...
if st.button("Ask the library", type="primary", use_container_width=True):
    if question.strip():
        ...
        st.info("Enter a question or choose one of the suggestions above.")

for message in st.session_state.messages:
    if message["role"] == "user":
        st.markdown(
            f'<div class="chat-user"><div class="role-label">You</div>{message["content"]}</div>',
            unsafe_allow_html=True,
        )
        continue

    refused_class = " refused" if message.get("refused") else ""
    st.markdown(
        f'<div class="chat-answer{refused_class}"><div class="role-label">Student AI Library</div>',
        unsafe_allow_html=True,
    )
...
st.markdown('<div class="footer">Student AI Library • Grounded RAG Demonstration</div>', unsafe_allow_html=True)
